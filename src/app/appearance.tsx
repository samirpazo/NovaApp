import { ArrowLeft, Check, Palette, Save, SlidersHorizontal } from 'lucide-react-native';
...
import { NOVA_COLORS, saveOrQueueAppearance, useAppearanceStore } from '@/theme/appearance';
...
export default function AppearanceScreen() {
  ...
  const [theme, setTheme] = React.useState(stored.Theme === 'light' ? 'light' : 'dark');
  ...
  React.useEffect(() => () => {
    discardPreview();
    setColorScheme(storedRef.current.Theme === 'light' ? 'light' : 'dark');
  }, [discardPreview, setColorScheme]);
  ...
  return <SafeAreaView className="flex-1 bg-background"><ScrollView contentContainerClassName="mx-auto w-full max-w-5xl gap-4 px-4 pb-20 pt-3">
    <View className="flex-row items-center gap-3"><Button variant="ghost" size="icon" onPress={() => { discardPreview(); setColorScheme(stored.Theme === 'light' ? 'light' : 'dark'); router.back(); }} accessibilityLabel="Volver"><ArrowLeft size={20} className="text-foreground" /></Button><View><Text className="font-poppins-bold text-xl">Personalización</Text><Text className="text-xs text-muted-foreground">Apariencia de Nova en este dispositivo</Text></View></View>
