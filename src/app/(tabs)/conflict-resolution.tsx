...
import { AlertTriangle, ArrowLeft, CheckCircle2, Cloud, RefreshCw, Smartphone } from 'lucide-react-native';
      ...
      <ScrollView contentContainerClassName="mx-auto w-full max-w-4xl gap-3 pb-24 pt-4">
        <View className="flex-row items-center gap-2">
          ...
          <View className="min-w-0 flex-1">
            <Text className="text-lg font-poppins-semibold">Resolver conflictos</Text>
            <Text variant="caption">{conflicts.length} decisión{conflicts.length === 1 ? '' : 'es'} pendiente{conflicts.length === 1 ? '' : 's'}</Text>
          </View>
          <Button variant="ghost" size="icon" className="h-8 w-8" onPress={() => void getSyncConflicts()} accessibilityLabel="Actualizar">
            <RefreshCw size={17} className="text-foreground" />
          </Button>
        </View>

        {error ? (
          <View className="flex-row items-start gap-2 rounded-md bg-destructive/10 px-3 py-2" role="alert">
            <AlertTriangle size={15} className="mt-0.5 text-destructive" />
            <Text className="min-w-0 flex-1 text-xs text-destructive">{error}</Text>
          </View>
        ) : null}
        {!conflicts.length ? (
          <View className="flex-row items-center gap-3 border-y border-border py-4">
            ...
            <View className="gap-0.5">
              ...
              <Text variant="caption">Los datos locales y del servidor ya no requieren decisiones.</Text>
            </View>
          </View>
            ...
            <Card key={key} className="overflow-hidden p-0">
              <View className="gap-1.5 border-b border-border bg-muted/35 p-4">
                <View className="flex-row items-center justify-between gap-3">
                  <Text className="text-sm font-poppins-semibold">{conflict.Resource}</Text>
                  <View className="flex-row items-center gap-1.5 rounded-md bg-warning/10 px-2.5 py-1.5">
                    <AlertTriangle size={13} className="text-warning" />
                    <Text className="text-xs font-poppins-semibold text-warning">Requiere decisión</Text>
                  </View>
                </View>
                <Text className="text-xs text-muted-foreground" numberOfLines={1} selectable>{conflict.SyncId}</Text>
              </View>
              ...
              <View className="gap-3 border-t border-border bg-muted/20 p-4 md:flex-row">
                <Button className="min-h-10 flex-1 px-3 py-2" variant="outline" disabled={busyKey === key} onPress={() => void resolveWithServer(conflict)}>
                  {busyKey === key ? <ActivityIndicator size="small" /> : <Cloud size={16} className="text-foreground" />}
                  <Text className="text-sm">Usar versión del servidor</Text>
                </Button>
                <Button className="min-h-10 flex-1 px-3 py-2" disabled={busyKey === key} onPress={() => void resolveWithLocal(conflict)}>
                  {busyKey === key ? <ActivityIndicator size="small" color="white" /> : <Smartphone size={16} className="text-primary-foreground" />}
