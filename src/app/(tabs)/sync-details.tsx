...
type LocalCounts = Record<
  (typeof SYNC_RESOURCES)[keyof typeof SYNC_RESOURCES],
  number
>;
...
async function readLocalCounts(): Promise<LocalCounts> {
  const entries = await Promise.all(
    Object.values(SYNC_RESOURCES).map(
      async (resource) =>
        [resource, await database.get(resource).query().fetchCount()] as const,
    ),
  );
...
export default function SyncScreen() {
  ...
  const confirmServerRecovery = () => {
    ...
    Alert.alert(
      syncRecoveryConfirmation.title,
      syncRecoveryConfirmation.message,
      [
  ...
  return (
    <SafeAreaView className="flex-1 bg-background">
      <ScrollView
        contentContainerClassName="mx-auto w-full max-w-3xl gap-3 pb-24 pt-3"
      ...
      >
        ...
        <Card className="gap-3 p-3">
          ...
          <Button
            className="h-9"
            disabled={!ready || sync.Status === 'syncing'}
          ...
          >
            ...
            />
            <Text>
              {sync.Status === 'syncing' ? 'Sincronizando...' : 'Sincronizar'}
            </Text>
          </Button>
          <Button
            variant="outline"
          ...
          <Text variant="caption">
            Acción técnica: vuelve a descargar el estado completo y conserva
        ...
        <View className="flex-row overflow-hidden rounded-md border border-border bg-card">
          ...
          <View className="min-w-0 flex-1 gap-0.5 p-3">
            <AlertTriangle
              size={15}
              className={
                conflictCount ? 'text-warning' : 'text-muted-foreground'
              }
            />
        ...
        {sync.LastPull ? (
          ...
          >
            ...
            {((sync.LastPull.DurationMs ?? 0) / 1000).toFixed(1)} s ·{' '}
            {sync.LastPull.Pages} página{sync.LastPull.Pages === 1 ? '' : 's'}
          </Text>
        ) : null}
        ...
        <Button
          ...
          onPress={() => router.push('/conflict-resolution')}
        >
          <AlertTriangle
            size={16}
            className={conflictCount ? 'text-warning' : 'text-muted-foreground'}
          />
          <View className="min-w-0 flex-1 items-start">
            <Text variant="small">Conflictos</Text>
            <Text variant="caption">
              {conflictCount} pendiente{conflictCount === 1 ? '' : 's'}
            </Text>
          </View>
          <ChevronRight
            size={16}
        ...
        <Card className="p-3">
          ...
          {Object.entries(counts).map(([resource, count]) => (
            <View
              key={resource}
              className="flex-row items-center justify-between border-b border-border py-2 last:border-b-0 last:pb-0"
            >
              <Text variant="caption">
                {resourceLabels[resource as keyof LocalCounts]}
              </Text>
        ...
        {sync.LastPull ? (
          <Text
            ...
            className="text-center"
          >
            Última ejecución: {formatters.datetime(sync.LastPull.FinishedAt)}
          </Text>
        ) : null}
      </ScrollView>
    </SafeAreaView>
