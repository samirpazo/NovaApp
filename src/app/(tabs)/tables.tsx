...
interface TableRow extends NCrudRow {
  TabID: number;
...
const columns: NCrudColumn<TableRow>[] = [
  ...
  { key: 'TabTableNumber', title: 'Mesa', width: 90, align: 'center' },
  ...
  { key: 'TabShape', title: 'Forma', width: 140 },
];
const toRow = (model: RstTableListItem): TableRow => ({
  id: model.LocalId,
...
export default function TablesScreen() {
  ...
  React.useEffect(() => {
    const subscription = rstTableQueries.observeActive(
      (records) => {
        setModels(records);
        setLoading(false);
      },
      (reason) => {
        setError(
          reason instanceof Error
            ? reason.message
            : 'No se pudieron leer las mesas locales.',
        );
        setLoading(false);
      },
    );
    ...
    <SafeAreaView className="flex-1 bg-background">
      <ScrollView contentContainerClassName="mx-auto w-full max-w-6xl gap-4 pb-24 pt-4">
        ...
        <NCrud
          ...
          loading={loading}
          readOnly
          searchPlaceholder="Número, capacidad o forma"
          searchText={(row) =>
            `${row.TabTableNumber} ${row.TabCapacity} ${row.TabShape}`
          }
        />
      </ScrollView>
    </SafeAreaView>
