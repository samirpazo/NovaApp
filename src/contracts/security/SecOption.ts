...
const SecOptionSchemaBase = z.object({
  ...
  OptCode: z.string().nullish().transform((value) => value ?? ''),
  OptName: z.string().nullish().transform((value) => value ?? ''),
  OptIcon: z.string().nullish().transform((value) => value ?? ''),
  OptIsMobile: z.boolean(),
  OptParent: z.number().int().nullish().transform((value) => value ?? null),
