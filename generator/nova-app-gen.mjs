...
for (const key of ['module', 'feature', 'entity', 'model', 'resource', 'pk']) {
  if (!/^[A-Za-z][A-Za-z0-9]*$/.test(args[key]))
    throw new Error(`--${key} debe ser un identificador válido.`);
}

const access = args.access ?? 'ReadWrite';
if (!['ReadWrite', 'ReadOnly'].includes(access))
  throw new Error('--access debe ser ReadWrite o ReadOnly.');

const modelFile = path.join(root, 'src/database/models', `${args.entity}.ts`);
if (!fs.existsSync(modelFile))
  throw new Error(`No existe el modelo: ${modelFile}`);
const modelSource = fs.readFileSync(modelFile, 'utf8');
const fields = [
  ...modelSource.matchAll(/@(field|text)\('([^']+)'\)\s+(\w+):\s+([^;]+);/g),
].map((match) => ({
  column: match[2],
  name: match[3],
  type: match[4].trim(),
}));
if (!fields.some((field) => field.name === args.pk))
  throw new Error(`El PK ${args.pk} no existe en ${args.model}.`);
...
const sortableFields = sortable
  .map(
    (field) =>
      `    ${field}: ${q(fields.find((item) => item.name === field).column)},`,
  )
  .join('\n');
const observedFields = fields
  .map((field) => q(field.column))
  .concat(q('SecStatus'))
  .join(', ');
const inputFields = fields.filter((field) => field.name !== args.pk);
const applyFields = inputFields
  .map((field) => `      record.${field.name} = input.${field.name};`)
  .join('\n');
...
${
  access === 'ReadWrite'
    ? `export interface Save${args.entity}Input {
  LocalId?: string;
  UserId: number;
...
${
  access === 'ReadWrite'
    ? `export const ${args.feature}Queries = {
  find(LocalId: string) {
